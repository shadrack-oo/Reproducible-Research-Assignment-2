---
title: "Assignment 2-Reproducible Reaserch"
author: "Shadrack Badia"
date: "5/28/2021"
output: html_document
---
## Impact of Severe Weather Events on  Health and Economy in the United States
...
# Load the dataset
stormDatainit <- read.csv("StormData.csv.bz2")
...
```{r}
if (dim(stormDatainit)[2] == 37) {
    stormDatainit$year <- as.numeric(format(as.Date(stormDatainit$BGN_DATE, format = "%m/%d/%Y %H:%M:%S"), "%Y"))
}
hist(stormDatainit$year, breaks = 30)
```  

Based on the above histogram, we see that the number of events tracked starts to significantly increase around 1995. 

The reason for this could be a high density of population leading to a tangibly higher rate infrastructure and people occupying them.

So, we use the subset of the data from 1990 to 2011 to get most out of good records.

```{r}
storm <- stormDatainit[stormDatainit$year >= 1995, ]
dim(storm)
```
...
```{r}
sortHelper <- function(fieldName, top = 15, dataset = stormDatainit) {
